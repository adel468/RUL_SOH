...
def main() -> int:
    parser = argparse.ArgumentParser(description='Verify the public ablation/statistical result files used by the manuscript.')
    ...
    status = {
        ...
        "description": 'Verify the public ablation/statistical result files used by the manuscript.',
        "expected_results": str(expected_root),
        "required_files": required,
        "missing_files": missing,
        "status": "PASS" if not missing else "FAIL",
    }
    status_path = status_dir / '04_ablation_and_statistics_status.json'
