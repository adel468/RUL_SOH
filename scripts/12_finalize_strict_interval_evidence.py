...
def main() -> int:
    parser = argparse.ArgumentParser(description='Verify the final selected-configuration and interval-safety result files.')
    ...
    required = [
    "paper_selected_configurations.csv",
    "paper_summary_by_dataset.csv",
    ...
    status = {
        ...
        "description": 'Verify the final selected-configuration and interval-safety result files.',
        "expected_results": str(expected_root),
        "required_files": required,
        "missing_files": missing,
        "status": "PASS" if not missing else "FAIL",
    }
    status_path = status_dir / '12_final_interval_evidence_status.json'
